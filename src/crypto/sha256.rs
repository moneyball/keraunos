...
//!
//! Lightning leans on SHA-256 everywhere: txids (doubled), payment hashes,
//! the BOLT 8 handshake transcript, key derivation tweaks, HKDF/HMAC.
...
pub struct Sha256 {
    ...
    buf: [u8; 64],
    buf_len: usize,
    /// Total bytes hashed so far (message length for padding).
    total: u64,
}
...
impl Sha256 {
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        ...
        self.total = 0; // already accounted; update() below must not matter
...
mod tests {
    ...
    fn nist_vectors() {
        ...
        for (input, want) in cases {
            assert_eq!(hex::encode(&Sha256::digest(input)), want);
        }
    }
    ...
    // Incremental hashing must agree with one-shot for awkward chunkings.
    #[test]
    fn incremental_matches_oneshot() {
        let data: Vec<u8> = (0..1037u32).map(|i| (i * 31 % 251) as u8).collect();
        for chunk in [1usize, 3, 63, 64, 65, 128, 1000] {
            let mut h = Sha256::new();
